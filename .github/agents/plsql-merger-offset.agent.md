...
- **`target_table_meta`** from the registry: `row_count`, `size_mb`, and `indexes` list

**How to use `target_table_meta`:**
- `indexes` — when writing the consolidated procedure's WHERE clauses and JOIN conditions,
  verify the predicate columns are index-backed. If a differing filter column (from the diff table)
  is NOT indexed, flag it as a DBA candidate in the change summary.
- `row_count > 10M` or `size_mb > 10,000` — add `/*+ APPEND */` to the bulk INSERT in the
  consolidated procedure and note undo space requirements in the change summary.
- State the table size prominently in the change summary so the developer can estimate runtime.
