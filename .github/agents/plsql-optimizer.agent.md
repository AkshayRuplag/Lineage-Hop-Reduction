...
3. **Extract table metadata from the registry entry.** Read `table_metadata.target_tables` and
   `table_metadata.source_tables`. Build an **Index Profile** table:

```
### Index Profile (from registry — Table_Info.xlsx + Index_Allobject.xlsx)

| Table | Role | Rows | Size (MB) | Indexes | Indexed Columns |
|-------|------|-----:|----------:|--------:|----------------|
| <tbl> | TARGET | <N> | <N> | <N> | <col1>, <col2> (NONUNIQUE); <pk_col> (UNIQUE) |
| <src> | SOURCE | <N> | <N> | <N> | <col1> (NONUNIQUE) |
```

   Use this profile throughout Phase 3:
   - When checking join keys: cross-reference against actual indexed columns
   - When checking non-sargable predicates: note if the filtered column IS indexed (higher priority fix)
   - When generating index candidates: only suggest NEW indexes not already in the profile
   - When assessing `row_count`: large tables (>5M rows) escalate anti-pattern severity to HIGH

4. Build the "already handled" summary, **including validator comments for each applied rec**:
...
|----------|-------------|---------------|-------------------|
| P. Cursor UPDATE | M-0058 | 8 BULK COLLECT cursors folded into INSERT SELECT | Can be included on initial load |
| Q. Hardcoded     | M-0053 | CASE → REF_LINE_OF_BUSINESS_GROUP_MAP | *(empty)* |
| O. GTT           | M-0031 | DIM_PLAN_DESIGN_DIRECTORY_R_GTT eliminated | *(empty)* |
...
...
**First: check the registry Index Profile (built in Phase 2).** The registry already contains
complete index metadata for this package's tables, sourced from `Index_Allobject.xlsx`.

1. For each join key in the SQL, look it up in the Index Profile:
   - If the column IS indexed → note "index-backed" next to the join — no index candidate needed
   - If the column is NOT indexed AND the table has >100K rows → flag as an **Index Candidate** with HIGH priority
   - If the column is NOT indexed AND `has_unique_index = false` on the target table → flag for DBA primary key / unique constraint review
2. For each non-sargable predicate found: check whether the underlying column is indexed — if yes, the fix is higher priority (rewriting the predicate would immediately benefit from the existing index).
3. Identify no more than **4 evidence-based** new index opportunities (gaps in the existing profile). For each:
   - Specific column(s) and table
   - Evidence (which query/cursor drives this)
   - Expected benefit and selectivity assumption
   - State whether the table already has other indexes (from registry) and how this one differs
   - Mark as **Candidate** only when the table itself is missing from the registry Index Profile
...
<index profile table from Phase 2 step 3 — include even for Mode B if table_metadata is present>
...
- Index candidates (new gaps only, not already-indexed columns):
