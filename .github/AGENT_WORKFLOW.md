...
- **Table-level metadata** (`table_metadata`): row counts, size in MB, column counts, and full index profiles for every source and target table referenced by the package — sourced from `Table_Info.xlsx` and `Index_Allobject.xlsx`
- **94 SQL packages** total: 47 recommendation-driven + 47 standalone candidates
    ...
    "target_tables": [
      {
        ...
        "indexes": [
          { "index_name": "RPT_POLICY_DTL_R_PK", "uniqueness": "UNIQUE", "columns": ["N_POLICY_SK_R", ...] },
          { "index_name": "RPT_POLICY_DTL_R_IDX1", "uniqueness": "NONUNIQUE", "columns": ["N_CLAIM_SK_R"] }
        ]
      }
    ],
    "source_tables": [ { "table_name": "FCT_GRP_POLICY_R", "row_count": 12403871, ... } ],
    "summary": {
      "total_target_rows": 4821034,
      "total_source_rows": 12403871,
      "total_target_size_mb": 24576.0,
      "total_source_size_mb": 9830.4,
      "total_target_indexes": 3,
      "total_source_indexes": 5,
      "meta_coverage": "2/2 tables in Table_Info"
    }
  },
  "recommendations": [
    {
      ...
      "target_table_meta": {
        ...
        "index_count": 3,
        "has_unique_index": true,
        "indexes": [ { "index_name": "...", "uniqueness": "UNIQUE", "columns": [...] } ]
      },
      "gap_consumers": [
        "DIM_GRP_CLAIM_POLICY_DETAIL_EMP_MV_SSL"
      ],
      "all_consumers": ["PKG_GRP_LOAD_RPT_CLAIM_SUM_R", "DIM_GRP_CLAIM_POLICY_DETAIL_EMP_MV_SSL"],
      "has_consumer_gaps": true,
      "cascade_chain": [
        {
          "consumer": "DIM_GRP_CLAIM_POLICY_DETAIL_EMP_MV_SSL",
          "also_targets": "M-0105",
...
> **Schema version 1.5** — `table_metadata`, `target_table_meta`, `gap_consumers`, `all_consumers`, `cascade_chain`, and `has_consumer_gaps` are present on all packages and recs when `Table_Info.xlsx`, `Index_Allobject.xlsx`, and `script_summaries_all.json` are available.

> **Consumer gap fields explained:**
> - `gap_consumers` — scripts that reference the global target object in JOIN/subquery references, **missed by FROM-clause-only lineage analysis**. Source: `script_summaries_all.json INPUT_DEPENDENCIES`.
> - `all_consumers` — union of `sql_objects_called` + `gap_consumers` (the complete impact set).
> - `has_consumer_gaps` — quick flag: `true` when at least one gap consumer exists.
> - `cascade_chain` — gap consumers that are ALSO the target of another global E./O. rec. Agents must sequence elimination carefully when this is non-empty.
...
# Override table / index metadata sources (defaults auto-detected)
python generate_pipeline_registry.py --table-info "path/to/Table_Info.xlsx"
python generate_pipeline_registry.py --index-info "path/to/Index_Allobject.xlsx"

# Override script summaries source
python generate_pipeline_registry.py --script-summaries "path/to/script_summaries_all.json"

# Skip individual enrichment steps
python generate_pipeline_registry.py --no-table-meta       # skip table/index metadata
python generate_pipeline_registry.py --no-consumer-gaps    # skip gap consumer analysis
```
