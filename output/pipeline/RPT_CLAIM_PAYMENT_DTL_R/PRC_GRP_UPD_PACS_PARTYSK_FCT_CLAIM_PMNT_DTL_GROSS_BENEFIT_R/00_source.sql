create or replace procedure PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R
as
...
ln_start_time := ATOMIC.LOG_TIME(ln_start_time, '1. atomic.PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R Entered-:'
, ld_sysdate);
...
from
  (select 
    ...
    ||mv.n_parent_objectnum_r         
    ||mv.n_claim_coverage_group_sk_r  
    ||mv.n_source_system_key_r        
  ...
  from fct_claim_pmt_dtl_gross_benefit_partysk_pacs_mv MV
  ) f
  WHERE RNK=1
    --and rownum <1000
  )
LOOP
   UPDATE FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R F
      SET  n_party_sk_r           =i.n_party_sk_r          
		  ...
		  ,v_last_modified_by_r   ='PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R'
...
END LOOP;
ln_start_time := ATOMIC.LOG_TIME(ln_start_time, '1.z Number of records updated :->'||ln_cnt||':-'||lc_row_id||' from procedure atomic.PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R and Exiting from the procedure-:'
, ld_sysdate);

EXCEPTION
WHEN OTHERS THEN
lc_errmsg:=SUBSTR(SQLERRM,1,4000);
ln_start_time := ATOMIC.LOG_TIME(ln_start_time, '1.z. Exception - Error in atomic.PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R Exit-:'||lc_errmsg
                 , ld_sysdate);
RAISE_APPLICATION_ERROR (-20343, 'Raise PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R Error -'||lc_errmsg);

END;

begin

V_TABLE_NAME_R :='FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME';

gc_trcmsg:='1. Drop and recreate table FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME';
...
SELECT COUNT(1) INTO N_EXISTS FROM USER_TABLES WHERE TABLE_NAME ='FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME';

  IF N_EXISTS<>0 THEN
   EXECUTE immediate 'DROP TABLE FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME';
  END IF;

EXECUTE IMMEDIATE 'CREATE TABLE FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME AS
SELECT N_CLAIM_SK_R,N_CLAIM_COVERAGE_SK_R,N_PARENT_OBJECTNUM_R, N_CLAIM_COVERAGE_GROUP_SK_R,N_SOURCE_SYSTEM_KEY_R,
		N_SOURCE_VERSION_SEQ_NUMBER_R,v_source_system_name_r, v_payee_first_name_r,v_payee_middle_name_r,v_payee_last_name_r FROM
        (SELECT  /*+PARALLEL(4)*/
        FBPR.N_CLAIM_SK_R          
		...
		FROM ATOMIC.FCT_BENEFIT_PAYMENT_R FBPR
            ...
            ,(select * from ATOMIC.dim_grp_party_r where v_active_status_r = ''Y'' and V_SOURCE_SYSTEM_NAME_R = ''PACS'')  pa --27-SEP-2023 CHANGES
		...
		MINUS
		SELECT N_CLAIM_SK_R,N_CLAIM_COVERAGE_SK_R,N_PARENT_OBJECTNUM_R, N_CLAIM_COVERAGE_GROUP_SK_R,N_SOURCE_SYSTEM_KEY_R,
		N_SOURCE_VERSION_SEQ_NUMBER_R,v_source_system_name_r, v_payee_first_name_r,v_payee_middle_name_r,v_payee_last_name_r
		FROM ATOMIC.FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R
		WHERE v_source_system_name_r = ''PACS''
		';

EXECUTE IMMEDIATE 'GRANT SELECT ON FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME TO ATOMIC_ALL_RO';


gc_trcmsg:='2. Table FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME created';
...
gc_trcmsg:='3. Merge table FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R_UPD_PAYEE_NAME with latest Payee Names';
 ...
 EXECUTE IMMEDIATE ' MERGE /*+APPEND*/INTO ATOMIC.FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R D
 USING (SELECT  * FROM ' || V_TABLE_NAME_R || '
 WHERE 1=1
 ) S
ON ( D.N_CLAIM_SK_R                       =S.N_CLAIM_SK_R                      
   and D.N_CLAIM_COVERAGE_SK_R            =S.N_CLAIM_COVERAGE_SK_R       
   and D.N_PARENT_OBJECTNUM_R             =S.N_PARENT_OBJECTNUM_R        
   and D.N_CLAIM_COVERAGE_GROUP_SK_R      =S.N_CLAIM_COVERAGE_GROUP_SK_R 
   AND D.N_SOURCE_SYSTEM_KEY_R            =S.N_SOURCE_SYSTEM_KEY_R 
   AND D.N_SOURCE_VERSION_SEQ_NUMBER_R    =S.N_SOURCE_VERSION_SEQ_NUMBER_R  
   )
   WHEN MATCHED THEN 
   UPDATE
   SET 
...
gc_trcmsg:='4. Table FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R merged with latest Payee Names';
...
DBMS_OUTPUT.PUT_LINE('PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R EXCEPTION WITH ERROR CODE AS '
             || SQLCODE
             || ' '
             || SQLERRM
             || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE
             ||'PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R'
             ||';');

    gc_errmsg :=SUBSTR(SQLERRM,1,4000);
    gc_trcmsg:='1.z Error in PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R: '||gc_errmsg;
	...
	pkg_grp_log_util.prc_update_log
  	...
  	RAISE_APPLICATION_ERROR(-20001,'Error in PRC_GRP_UPD_PACS_PARTYSK_FCT_CLAIM_PMNT_DTL_GROSS_BENEFIT_R.FCT_CLAIM_PAYMENT_DETAIL_GROSS_BENEFIT_R:->
