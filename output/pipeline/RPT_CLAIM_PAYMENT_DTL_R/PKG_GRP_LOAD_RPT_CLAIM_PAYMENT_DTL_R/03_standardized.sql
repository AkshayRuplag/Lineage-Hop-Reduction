...
***********************************************************************/
  --Global Constants
		...
		-- OPT-2: removed unused gt_start_time_insd_lp, gn_error_line, gn_loop_counter_r (orphaned by Kill/Fill comment-out)
    ...
    --open P_OUT_CURSOR for
		...
		LEFT JOIN
			(
				...
				SELECT MAX(N_CUST_PARTY_SK_R) AS N_CUST_PARTY_SK_R, N_POLICY_SK_R, N_VERSION_NUMBER_R
		...
		-- OPT-3: joined directly to MVW_PRODUCT_SK_LOOKUP_MIN_MV_SSL (dropped duplicate derived-table wrapper); kept as two joins since key paths differ (DIGCOV vs DGC_COV_GRP)
		LEFT JOIN ATOMIC.MVW_PRODUCT_SK_LOOKUP_MIN_MV_SSL AS MV_PRD_LKP
			ON  MV_PRD_LKP.n_claim_sk_r = DIGCD.n_claim_sk_r
			AND MV_PRD_LKP.v_claim_coverage_code_r = DIGCOV.v_claim_coverage_code_r

		LEFT JOIN ATOMIC.dim_grp_product_r AS DG_PRD_J
			ON 	MV_PRD_LKP.N_PRODUCT_SK_R = DG_PRD_J.N_PRODUCT_SK_R
		LEFT JOIN ATOMIC.MVW_PRODUCT_SK_LOOKUP_MIN_MV_SSL AS MV_PRD_LKI
			ON  MV_PRD_LKI.n_claim_sk_r = DIGCD.n_claim_sk_r
			AND MV_PRD_LKI.v_claim_coverage_code_r = DGC_COV_GRP.v_claim_coverage_code_r
		LEFT JOIN ATOMIC.dim_grp_product_r AS dim_grp_product_r_l
...
EXCEPTION
WHEN OTHERS THEN
    gv_errmsg :='SQLCODE: '||SQLCODE||' - '||SUBSTR(SQLERRM,1,4000);
...
IS
	   ...
	   /*TYPE lt_var_tbl_type IS TABLE OF RPT_CLAIM_PAYMENT_DTL_R%ROWTYPE INDEX BY BINARY_INTEGER;
        ...
        -- OPT-2: removed unused locals lt_insert_time, lv_rpt_table, ln_loop_counter, ln_rec_cnt, ln_idx_num (orphaned by Kill/Fill comment-out)
