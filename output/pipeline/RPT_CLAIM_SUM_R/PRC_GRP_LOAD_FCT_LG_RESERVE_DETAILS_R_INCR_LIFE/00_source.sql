create or replace PROCEDURE PRC_GRP_LOAD_FCT_LG_RESERVE_DETAILS_R_INCR_LIFE (
...
LC_TRCMSG:= LC_TRCMSG||'Getting the Previous Cycle date from DIM_TIME_R '||CHR(13);
     ...
     SELECT /*+PARALLEL(4)*/ D_CALENDAR_DATE_R INTO LD_MIS_DATE_PREV_R 
	 FROM
...
*/
  ...
  LC_TRCMSG := LC_TRCMSG
                 || CHR(13)
                 || '1. INSERT INTO THE TABLE FCT_LG_RESERVE_DETAILS_R_TMP:->'
                 || LN_START_TIME
                 || ' SECONDS';


	LC_TRCMSG := LC_TRCMSG||'Deleting the data from FCT_LG_RESERVE_DETAILS_R_TMP for V_RESERVE_TYPE_IND_R L '||CHR(13);			

	DELETE FROM FCT_LG_RESERVE_DETAILS_R_TMP WHERE V_RESERVE_TYPE_IND_R IN ('L'); --AND TRUNC(D_RESERVE_VALUATION_DATE_R)=TRUNC(LD_MIS_DATE_PREV_R);
	COMMIT;
	LC_TRCMSG := LC_TRCMSG||'Deleted the data for Life from FCT_LG_RESERVE_DETAILS_R_TMP'||CHR(13);	
	LC_TRCMSG := LC_TRCMSG||'Starting the load for Current Cycle Date to FCT_LG_RESERVE_DETAILS_R_TMP '||CHR(13);	


		INSERT  INTO FCT_LG_RESERVE_DETAILS_R_TMP
			...
			SELECT
					...
					FROM (
						...
						/*			where extract(month from TO_DATE( RS1.D_VALUATION_DATE_R , 'MM/DD/YYYY')) =extract (month from TO_DATE(LD_MIS_DATE_R, 'dd-mon-yy') )
									AND extract(YEAR from to_date( RS1.D_VALUATION_DATE_R , 'MM/DD/YYYY')) = EXTRACT (YEAR FROM to_date(LD_MIS_DATE_R, 'dd-mon-yy') )
									--where TO_CHAR(RS1.D_VALUATION_DATE_R, 'YYYYMM') = LN_REP_MON_R
...
/*									WHERE  EXTRACT(MONTH FROM TO_DATE( BE1.D_VALUATION_DATE_R , 'MM/DD/YYYY')) = EXTRACT (MONTH FROM to_date(LD_MIS_DATE_R, 'dd-mon-yy') )
									AND EXTRACT (YEAR FROM TO_DATE( BE1.D_VALUATION_DATE_R , 'MM/DD/YYYY')) = EXTRACT (YEAR FROM to_date(LD_MIS_DATE_R, 'dd-mon-yy') )
*/
					...
					--AND TO_CHAR(RS.D_VALUATION_DATE_R, 'YYYYMM') = TO_CHAR(BE.D_VALUATION_DATE_R, 'YYYYMM')--Commented to fix duplicate issue-429498
 ...
 LC_TRCMSG := LC_TRCMSG || CHR(13) || ' INSERTED DATA INTO THE TABLE FCT_LG_RESERVE_DETAILS_R_TMP:->'|| ( DBMS_UTILITY.GET_TIME - LN_START_TIME )|| ' SECONDS';
...
ELSE
 ...
 /*
 LC_TRCMSG := LC_TRCMSG || CHR(13) ||' Taking the backup of  STG_BEST_ESTIMATE_RESERVES_R Table '||CHR(13);	 
 ...
 --CREATE TABLE STG_BEST_ESTIMATE_RESERVES_R_BKP_PREV_MONTH AS SELECT * FROM STG_BEST_ESTIMATE_RESERVES_R;

 LC_TRCMSG := LC_TRCMSG || CHR(13) || ' Backup table for STG_BEST_ESTIMATE_RESERVES_R has been created ' || CHR(13);		
...
--GC_TRC_MSG:='FINAL ERROR MESSAGE:->'||LC_SQLCODE||'-'||LC_SQLERRM;
        INSERT INTO ATOMIC.PRCS_GRP_TBL_LOAD_DEBUG_TRC (
            ...
            'PRC_GRP_LOAD_FCT_LG_RESERVE_DETAILS_R_INCR_LIFE',
            'PRC_GRP_LOAD_FCT_LG_RESERVE_DETAILS_R_INCR_LIFE',
            NULL,
            NULL,
            'WHEN OTHERS RAISED IN PRC_GRP_LOAD_FCT_LG_RESERVE_DETAILS_R_INCR_LIFE :->'
            || LC_TRCMSG
            || LC_SQLCODE
            || '->'
            || LC_SQLERRM,
        ...
        COMMIT;
       /* RAISE_APPLICATION_ERROR(-20001, 'OTHERS-ERROR IN PRC_GRP_LOAD_FCT_LG_RESERVE_DETAILS_R_INCR_LIFE:->' || LC_SQLERRM);*/
