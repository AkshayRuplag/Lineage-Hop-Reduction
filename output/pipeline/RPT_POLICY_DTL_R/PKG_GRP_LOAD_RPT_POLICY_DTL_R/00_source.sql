...
BEGIN
    ...
    --TRUNCATE TABLE DIM_PLAN_DESIGN_DIRECTORY_R_GTT PURGE;
    INSERT /*+APPEND PARALLEL(TGT,4)*/
      INTO DIM_PLAN_DESIGN_DIRECTORY_R_GTT TGT
        ...
        SELECT T4804933.N_PLAN_DESIGN_SK_R
         ...
         WHERE T4804933.v_active_status_r = 'Y'
           AND T4804933.v_coverage_code_r IN ('IDTHEFT','IDTHEFTEFFDATE','PSINDICATOR','ELIMPERIOD','EAIND', 'EAP'
    			                             ,'BEREAVE','EAP_EFF_DATE','BEREAVEDATE','BENEPERIOD', 'BENEFITDURATION'  --24-06-2025 'BENEPERIOD', 'BENEFITDURATION' ADDED
    		                                  )
    		AND EXISTS (SELECT 1
        ...
        gv_trcmsg:='9.3.2 Exit from in prc_load_data_dim_gtt.Total records deleted : '||ln_rec_cnt;
...
WHEN OTHERS THEN
	...
	gv_trcmsg:='9.3.z Error in prc_load_data_dim_gtt'||gv_errmsg;
  ...
  CURSOR cur_upd_nxtrenewaldt_col
   ...
   WHERE n_yearmonth_r = gn_current_month;
	...
	SELECT fgpr.n_policy_sk_r     AS n_policy_sk_r
	  ...
	  from  fct_grp_policy_r fgpr,
		   ...
		   (SELECT N_SOURCE_SYSTEM_KEY_R
				 ...
				 , V_BUS_OBJ_STATUS_R
			  FROM fct_grp_transactions_r
		     WHERE V_VERSION_TYPE_R in('RENEWAL', 'NEWBUS')
		  order by N_TXN_VERSION_NUMBER_R desc
		    )ft2
	...
	WHERE a.n_policy_sk_r = b.n_policy_sk_r
	  AND TO_CHAR(a.D_CYCLE_DATE_R, 'YYYYMM') = gn_current_month
	  AND b.n_yearmonth_r = gn_current_month
	  AND EXISTS ( SELECT 1
			         FROM rpt_policy_dtl_r policy
			        WHERE policy.n_policy_sk_r = a.n_policy_sk_r
			          AND policy.n_yearmonth_r = gn_current_month
		         )
...
---Update PROCEDURE
    ...
    /*START:NEW LOGGING MECHANISM CHANGES*/
	gv_trcmsg:='9.3 Start: Call Procedure prc_load_data_dim_gtt';
	gt_start_time := SYSTIMESTAMP;
	PKG_GRP_LOG_UTIL.prc_ins_prcs_job_log_message_r(
					 p_job_id_r                     => gn_out_job_id
					,p_batch_id_r                   => gn_sysdt_batchid
	...
	prc_load_data_dim_gtt;

	/*START: NEW LOGGING MECHANISM CHANGES*/

	gt_end_time := SYSTIMESTAMP;
	  gv_trcmsg :='9.4 End: Called Procedure prc_load_data_dim_gtt';
		PKG_GRP_LOG_UTIL.prc_ins_prcs_job_log_message_r(
					 p_job_id_r                    	=> gn_out_job_id
					,p_batch_id_r                  	=> gn_sysdt_batchid
					,p_message_type_r              	=> GV_MESSAGE_TYPE
	...
	LOOP
     ...
     ld_theft_dt:=null;
	    BEGIN
		   SELECT TO_DATE(LT_VAR_UPD_TBL_THEFT_DT_TYP_REC(x).v_override_description_r,'MM/DD/YYYY')
		     INTO ld_theft_dt
			 FROM DUAL;
    ...
    LOOP
        FETCH cur_upd_agencycode_cols BULK COLLECT INTO lt_var_upd_tbl_agencycode_typ_rec LIMIT 10000;
	...
	-- End : Kill/Fill Changes 5th May 2026: Added following
     ...
     -- , CAST(NULL AS VARCHAR2(50))                                                           AS V_PLAN_DURATION_R        -- 24-06-2025 added as per as FDM Reqt  11-07-2025 Commented as per as not required
	    ...
	    ,(SELECT *
	       ...
	       WHERE a.T_EVENT_TIMESTAMP_R = (SELECT MAX(T_EVENT_TIMESTAMP_R)
		                                    FROM ATOMIC.FCT_GRP_BILLING_POLICY_DTL_R B
                                           WHERE A.N_POLICY_SK_R = B.N_POLICY_SK_R
											 AND B.D_DELETE_DATE_R IS  NULL
	                                     )
