...
def call_dataFlowAnalyzer(args):
    ...
    # JAXB was removed from Java 11+; include standalone JAXB JARs for Java 11+17 compatibility
    import os as _os
    _sep = ";" if _os.name == "nt" else ":"
    _jar_dir = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "jar")
    _jars = [
        "gudusoft.gsqlparser-2.8.5.8.jar",
        "jaxb-api-2.3.1.jar",
        "jaxb-impl-2.3.3.jar",
        "jaxb-core-2.3.0.1.jar",
        "activation-1.1.1.jar",
    ]
    _cp = _sep.join(_os.path.join(_jar_dir, j) for j in _jars)
    jar = f"-Djava.class.path={_cp}"
    ...
    try:
        ...
        if indexOf(args, "/version") != -1:
            print("Version: " + str(DataFlowAnalyzer.getVersion()))
            print("Release Date: " + str(DataFlowAnalyzer.getReleaseDate()))
            return
